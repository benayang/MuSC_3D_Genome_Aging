...
projdir="C:\\Users\\benjy\\Dropbox (University of Michigan)\\ENGIN-Lab Notes\\Lab Notes\\Lab Notes Benjamin\\Hi-C\\04_FANC\\track_coverage"
...
comp.switch.test = df %>% 
  pivot_longer(cols=c("ATAC","H3K4me3"),names_to="BW",values_to="RPKM") %>%
  filter(Group %in% c("AtoB","BtoA","StaticA","StaticB")) %>%
  mutate(log2RPKM=log2(RPKM+1),
         Group = factor(Group, levels=c("StaticA", "StaticB", "BtoA", "AtoB"))) %>%
  group_by(Group,BW) %>%
  wilcox_test(RPKM~Age) %>%
...
df %>% 
  ...
  stat_summary(aes(group=Age), fun.data="mean_se",geom="errorbar", color="black", width=0.5, position=position_dodge(0.9)) +
  ...
  scale_fill_manual(values=rev(pal_nejm()(2)), labels=c("young"="Young","aged"="Aged")) +
  stat_pvalue_manual(comp.switch.test, tip.length=0.005, label.size=5, label="p.label") +
...
ggsave(file.path(projdir,"compartment_switch_sum_track_coverage.png"),dpi=300,width=4.25,height=5)
...
compartment.df = df %>% 
  pivot_longer(cols=c("ATAC","H3K4me3"),names_to="BW",values_to="RPKM") %>%
  dplyr::filter(Group %in% c("young.A","young.B","aged.A","aged.B")) %>%
