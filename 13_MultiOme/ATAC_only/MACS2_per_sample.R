...
# Import data
aged = readRDS("/nas/homes/benyang/HiC/13_MultiOme/RawData/aged_raw_seurat.RDS")
young = readRDS("/nas/homes/benyang/HiC/13_MultiOme/RawData/young_raw_seurat.RDS")
young_v2 = readRDS("/nas/homes/benyang/HiC/13_MultiOme/RawData/young_v2_raw_seurat.RDS")

outdir = "/nas/homes/benyang/HiC/13_MultiOme/ATAC_only/MACS2_sample_peaks"
seurat_outdir = "/nas/homes/benyang/HiC/13_MultiOme/ATAC_only"
...
young = CreateChromatinAssay(
        counts = GetAssayData(young, assay="ATAC", slot="counts"), 
        fragments = "/nas/homes/benyang/HiC/13_MultiOme/RawData/Young/10x_analysis_5644-JL/Sample_5644-JL-1/atac_fragments.tsv.gz", max.lines = NULL)
young_v2 = CreateChromatinAssay(
    counts = GetAssayData(young_v2, assay="ATAC", slot="counts"), 
    fragments = "/nas/datasets/6098-JL/10x_analysis_6098-JL/Sample_6098-JL-1/atac_fragments.tsv.gz")
aged = CreateChromatinAssay(
    counts = GetAssayData(aged, assay="ATAC", slot="counts"), 
...
young_peak.counts = call_peaks(young, "young", blacklist, outdir)
saveRDS(young_peak.counts, file.path(outdir, "young_peak_counts.RDS"))

young_v2_peak.counts = call_peaks(young_v2, "young_v2", blacklist, outdir)
saveRDS(young_v2_peak.counts, file.path(outdir, "young_v2_peak_counts.RDS"))

aged_peak.counts = call_peaks(aged, "aged", blacklist, outdir)
...
young = CreateSeuratObject(counts = young_MACS2, assay="ATAC", project="Young")
saveRDS(young_MACS2, file.path(seurat_outdir, "young_raw_ATAC_obj.RDS"))

young_v2_MACS2 <- CreateChromatinAssay(
  counts = young_v2_peak.counts,
  fragments = "/nas/datasets/6098-JL/10x_analysis_6098-JL/Sample_6098-JL-1/atac_fragments.tsv.gz"
)
young_v2 = CreateSeuratObject(counts = young_v2_MACS2, assay="ATAC", project="Young_v2")
saveRDS(young_v2, file.path(seurat_outdir, "young_v2_raw_ATAC_obj.RDS"))

aged_MACS2 <- CreateChromatinAssay(
  counts = aged_peak.counts,
  fragments = "/nas/homes/benyang/HiC/13_MultiOme/RawData/Aged/10x_analysis_5422-JL/Sample_5422-JL-1/atac_fragments.tsv.gz"
)
aged = CreateSeuratObject(counts = aged_MACS2, assay="ATAC", project="Aged")
saveRDS(aged, file.path(seurat_outdir, "aged_raw_ATAC_obj.RDS"))
