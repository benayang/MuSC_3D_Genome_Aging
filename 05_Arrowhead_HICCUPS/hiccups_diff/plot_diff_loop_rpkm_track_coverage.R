...
projdir = "C:\\Users\\benjy\\Dropbox (University of Michigan)\\ENGIN-Lab Notes\\Lab Notes\\Lab Notes Benjamin\\Hi-C\\05_Arrowhead_HICCUPS\\hiccups_diff\\loop_domain_track_coverage\\diff_analysis"
...
for(f in files) {
  ...
  data.list[[fname]]$group = fname
  data.list[[fname]]$score_age = ifelse(grepl("aged_scores",fname,fixed=T), "Aged", "Young")
  data.list[[fname]]$loop_group = ifelse(grepl("non_diff_loop",fname,fixed=T), "Static", "Diff")
  data.list[[fname]]$loop_age = ifelse(grepl("per_aged",fname,fixed=T), "Aged", "Young")
}

data_df = bind_rows(data.list)
plt_data = data_df %>% 
  pivot_longer(cols=c(ATAC,H3K4me3), names_to="mod") %>%
  pivot_wider(id_cols = c(chr,start,end,loop_group,loop_age), names_from = c(mod, score_age), values_from = value) %>%
...
plt_data_test = plt_data %>% group_by(log2fc, loop_age) %>% wilcox_test(value ~ loop_group)
plt_data_test = plt_data_test %>% add_xy_position(x="loop_age", dodge=0.9)

ggplot(plt_data, aes(x=loop_age, y=ATAC_Aged)) +
  geom_boxplot(aes(fill=loop_group))

ggplot(plt_data, aes(x=loop_age, y=value)) +
  facet_wrap(~log2fc) +
